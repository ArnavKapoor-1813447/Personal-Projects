% this is my final code for the blackjack game which i decided to place as
% a seperate function due to the code complexity and length. This allowed
% me to alter and debug the code much easier thand before when i had added
% it directly into the main driver

%Actual blackjack game

function[user_capital, bet_initial] = final_blackjack(user_capital, bet_initial)
...
fprintf ("Current dealer total is %d \n",values(shuffle_card_dealer(1)))
 ...
 if total==21 %output for a blackjack
...
while dealer_total<17 %must keep hitting
    ...
    elseif dealer_total>total %if dealer hand higher than player
        ...
        fprintf("\nYour capital is now: %d\n ", user_capital)
        return
    elseif dealer_total<total&&dealer_total>=17 %if dealer hand lower than player
        fprintf("\nthats YOUR WIN, dont get too smug")
        user_capital= user_capital + (bet_initial);
        fprintf("\nYour capital is now: %d\n ", user_capital)
return
    elseif dealer_total==total && dealer_total >=17 
        fprintf("\nSTANDOFF, just like those old cowboy movies.")
        fprintf("\nYour capital is now: %d\n ", user_capital) %capital doesnt change, as bet is refunded
        return
    end
